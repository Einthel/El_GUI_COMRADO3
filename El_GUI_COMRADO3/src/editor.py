...
from LoadSave import (
    ...
    load_custom_buttons, add_custom_button, save_custom_buttons
)
...
class EditorWindow(QMainWindow):
    ...
    def _setup_ui_components(self):
        ...
        layout = original_widget.parentWidget().layout()

        if layout:
            # Создаем экземпляр нашего нового виджета
            self.custom_list_widget = DraggableListWidget(original_widget.parentWidget())
            
            # Копируем важные свойства
            self.custom_list_widget.setObjectName(original_widget.objectName())
            self.custom_list_widget.setMinimumSize(original_widget.minimumSize())
            self.custom_list_widget.setMaximumSize(original_widget.maximumSize())
            self.custom_list_widget.setTabletTracking(original_widget.hasTabletTracking())
            self.custom_list_widget.setFocusPolicy(original_widget.focusPolicy())
            self.custom_list_widget.setFrameShadow(original_widget.frameShadow())
            self.custom_list_widget.setSizeAdjustPolicy(original_widget.sizeAdjustPolicy())
            
            # Заменяем виджет в компоновке
            layout.replaceWidget(original_widget, self.custom_list_widget)
        ...
        original_font_combo = self.ui.fontComboBox
        font_layout = original_font_combo.parentWidget().layout()

        if font_layout:
            # Создаем наш кастомный комбобокс
            self.font_combo_box = QComboBox()
            for font_name in sorted(SAFE_FONTS):
                self.font_combo_box.addItem(font_name)
            
            # Копируем свойства
            self.font_combo_box.setMinimumSize(original_font_combo.minimumSize())
            self.font_combo_box.setMaximumSize(original_font_combo.maximumSize())

            # Заменяем виджет в компоновке
            font_layout.replaceWidget(original_font_combo, self.font_combo_box)
            original_font_combo.deleteLater() # Старый виджет удаляем
        ...
        # Код ниже больше не нужен, так как вкладка "Действие"
        # и ее содержимое уже корректно настроены в Editor.ui
        ...
        # Присваиваем self.action_tab для совместимости с остальным кодом
        self.action_tab = self.ui.Action_tab
    ...
    def _apply_shadow_effects(self):
        ...
        # Применяем стандартную тень к списку пресетов и группе редактирования
        apply_shadow(self.ui.Ready_Button_listWidget, blur_radius=15, x_offset=5, y_offset=5)
        apply_shadow(self.ui.Edit_Button_groupBox, blur_radius=15, x_offset=5, y_offset=5)
        
        # Для предпросмотра тень не нужна, так как она уже есть у родительского layout
        # Если QFrame `Button_example_layout` все же нужен с тенью, можно добавить:
        # apply_shadow(self.ui.Button_example_layout, blur_radius=15, x_offset=5, y_offset=5)
    ...
    def _create_new_page_widget(self, page_key):
        ...
        apply_shadow(button_frame, blur_radius=25, color=(0, 0, 0, 200))
        ...
        for i in range(1, 13):
            ...
            apply_shadow(button, blur_radius=15, x_offset=5, y_offset=5)
